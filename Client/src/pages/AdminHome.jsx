...
function AdminHome(props) {
  ...
  const [showAddItem, setShowAddItem] = useState(0);
  const [showUpdateItem, setShowUpdateItem] = useState(0);
  const [showFilter, setShowFilter] = useState(0);
  let [filterWatches, setFilterWatches] = useState([]);
  let [copyFilter, setCopyFilter] = useState([]);
  var [mem, setMem] = useState("");
  ...
  const callFunction = async () => {
    ...
    let allWatches = await readOnlyData("http://localhost:8000/v1/watch");
    let watchInStore = await readOnlyData(
      `http://localhost:8000/v1/store/${localStorage.getItem("id")}`
    );

    let member = await readOnlyData(
      `http://localhost:8000/v1/member/${localStorage.getItem("id")}`
    );

    allWatches = allWatches.data.watch;
    watchInStore = watchInStore.data.store[0].store_products;
    // remove the empty one
    watchInStore = watchInStore.filter((element) => element.product_id !== "");
    // return those watches that are in store
    let products = allWatches.filter((ele) =>
      watchInStore.filter((e) => e === ele)
    );
    setFilterWatches(products);
    setCopyFilter(products);

    setMem(member.data.member);

    // console.log("All watches : ", allWatches);
    // console.log("Watches in Store : ", watchInStore);
    // console.log("Objects : ", products);
    // console.log("filter : ", filterWatches);
  };

  const adminSearch = (search) => {
    console.log(search);
    if (search === "") {
      setFilterWatches(copyFilter);
    } else {
      const products = filterWatches.filter(
        (element) =>
          element.name.search(search) > -1 ||
          element.brand.search(search) > -1 ||
          element.status.search(search) > -1
      );
    ...
    <div class="app-container">
      ...
      {showUpdateItem === 1 ? (
        <UpdateItem
          ...
          setShowUpdateItem={setShowUpdateItem}
          id={prod}
        />
      ) : null}
        ...
        <ul class="sidebar-list">
          <li class="sidebar-list-item">
            <a href="#">
              ...
              >
                <path d="M3 9l9-7 9 7v11a2 2 1-2 2H5a2 2 0 0 1-2-2z" />
                <polyline points="9 22 9 12 15 22" />
              </svg>
              <span>My Cart</span>
            </a>
          </li>
          ...
          <li class="sidebar-list-item">
            <a href="#">
              ...
              >
                ...
                <polyline points="9 22 9 12 5 12 15 22" />
              </svg>
              <span>My Orders</span>
            </a>
          </li>
